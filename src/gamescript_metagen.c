...
static void generate_game_script_builtins_procedure_table(char* argv0, const char* outfile) {
...

    {
        /*
          ...
          okay too...
         */
        struct directory_listing listing = directory_listing_list_all_files_in(&arena, string_literal("./src"));

        static string blacklisted_files[] = {
            string_literal("./src/metagen.c"),
            string_literal("./src/generated_game_script_proc_table.c"),
        };

        for (unsigned entry_index = 0; entry_index < listing.count; ++entry_index) {
            string filename = string_concatenate(&arena, string_literal("./src/"), string_from_cstring(listing.files[entry_index].name));
...
int main(int argc, char** argv) {
    ...
    {
        generate_game_script_builtins_procedure_table(argv[0], "./src/generated_game_script_proc_table.c");
    }
    return 0;
}
